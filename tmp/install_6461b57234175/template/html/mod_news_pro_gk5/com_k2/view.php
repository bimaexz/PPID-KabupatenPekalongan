...
class NSP_GK5_com_k2_View extends NSP_GK5_View {
	...
	static function info($config, $item, $num = 1) {
		...
		if(
			...
			$info_category = ($config['category_link'] == 1) ? '<a class="nspCategory" href="'.static::categoryLink($item).'" target="'.$config['open_links_window'].'">'.$item['catname'].'</a>' : $item['catname'];
	        ...
	        $author_html = '<a class="nspAuthor" href="'.urldecode(JRoute::_(K2HelperRoute::getUserRoute($item['author_id']))).'" target="'.$config['open_links_window'].'">';
	        ...
	        //
	        $info_date = '<span class="nspDate">' .JHTML::_(' date', $item['date'], $config['date_format']) . '</span>';
	        //
	        $info_hits = '<span class="nspHits"> <i class="fa fa-heart-o"> </i> '.$item['hits'].'</span>';
