...
class NSP_GK5_View {
	...
	static function lists($config, $item, $num) {
		...
		if(isset($config['date']) == 0) {
			$nspdate = '<div class="nspListDate">' .JHTML::_(' date', $item['date'], $config['date_format']) . '</div>';
		}


		if(isset($config['category_link']) == 1) {
			$catname = '<a class="nspListCategory" href="'.static::categoryLink($item).'">';
		...
		$output_html .= $title . $text . $catname . $nspdate . $readmore;
