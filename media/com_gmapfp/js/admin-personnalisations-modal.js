...
(function () {
  ...
  /**
    ...
    * View element calls jSelectItem when a contact is clicked
    * jSelectItem creates the link tag, sends it to the editor,
  ...
  window.jSelectItem = function (id, title, catid, object, link, lang) {
    ...
    if (!Joomla.getOptions('xtd-personnalisations')) {
      // Something went wrong
      window.parent.Joomla.Modal.getCurrent().close();
      return false;
    }

    var _Joomla$getOptions = Joomla.getOptions('xtd-personnalisations'),
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    for (var i = 0, l = elements.length; l > i; i += 1) {
      ...
      elements[i].addEventListener('click', function (event) {
        ...
        if (functionName === 'jSelectPersonnalisation') {
          // Used in xtd_items
