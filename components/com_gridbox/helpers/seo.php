    ...
    protected function checkTag($tag, $match)
    {
        ...
        switch ($tag) {
            case 'Field \d+':
                ...
                $db->setQuery($query);
                $data = $db->loadObject();
                if ($data && ($data->field_type == 'radio' || $data->field_type == 'select' || $data->field_type == 'checkbox')) {
                    ...
                    $value = implode(', ', $fieldOption->title);
                ...
                } else if ($data && $data->field_type == 'date' || $value->field_type == 'event-date') {
                    $value = gridboxHelper::getPostDate($value->value);
                    ...
                    $value = gridboxHelper::preparePrice($value->value, $thousand, $separator, $decimals, 1);
                } else if ($data) {
                    $value = $data->value;
                }
                $tag = $match;
