...
class cssHelper
{
    ...
    {
        if (isset($obj->colors->default) && isset($obj->{'colors-bg'}->type)) {
            $obj->colors->default->type = $obj->{'colors-bg'}->type;
        }
        if (isset($obj->colors->default) && isset($obj->{'colors-bg'}->gradient)) {
    ...
    public function overlay($obj, $variable = null, $calc = null)
    {
        ...
        if ((!isset($obj->type) || $obj->type == 'color') && isset($obj->color)) {
            ...
            $this->css .= 'backdrop-filter: none;';
            $this->css .= '-webkit-backdrop-filter: none;';
        } else if (isset($obj->type) && $obj->type == 'blur') {
    ...
    {
        $str = '';
        if (isset($obj->background) && isset($obj->background->type) && $obj->background->type == 'video') {
    ...
    public function createPageRules($obj, $selector)
    {
        ...
        if (!empty($obj->shape->bottom->effect) && (isset($obj->shape->bottom->value) || isset($obj->shape->bottom->width)
                        || isset($obj->shape->bottom->height))) {
            $str .= $this->getShapeRules($selector, $this->object->shape->bottom, 'bottom');
        }
        if (!empty($obj->shape->top->effect) && (isset($obj->shape->top->value) || isset($obj->shape->top->width)
    ...
    public function getCheckoutFormRules($obj, $selector)
    {
        ...
        if (!empty($css)) {
            $str .= "body, .ba-checkout-form-fields, .ba-item-checkout-order-form {";
            $str .= $css;
            $str .= "}";
        }
    ...
    public function getSimpleGalleryRules($obj, $selector)
    {
        ...
        if (isset($this->object->images) &&
            ...
            foreach ($this->object->images as $ind => $image) {
                ...
                $str .= "background-image: url(".$this->setBackgroundImage($image).") !important;";
                $str .= '}';
            }
        }
        if (isset($obj->border)) {
