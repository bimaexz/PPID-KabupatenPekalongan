...
class store
{
        ...
        foreach ($items as $item) {
            ...
            if (!empty($renew->remind->value)) {
                ...
                foreach ($list as $obj) {
                    ...
                    $this->tags->{'[Expiration Date]'} = gridboxHelper::getPostDate($obj->expires);
    ...
    protected function checkSettings()
    {
        ...
        //unset($store->added);
        if (!isset($store->added)) {
            $store->added = [];
            $update = true;
        }
        if (!in_array('customer-review', $store->added)) {
            $str = gridboxHelper::readFile($path);
            $object = json_decode($str);
            foreach ($object->notifications as $notification) {
                ...
                $notification->body = gridboxHelper::readFile($dir.'customer-review.html');
                $store->notifications[] = $notification;
                $store->added[] = 'customer-review';
            }
            $update = true;
        }
        if ($update) {
    ...
    public function createAdminOrder($cart, $user_id)
    {
        $db = JFactory::getDbo();
        $order = new stdClass();
        $order->cart_id = 0;
        ...
        foreach ($cart->products as $obj) {
            $product = $this->getProductObject($obj, $order->id);
            $this->insertProduct($product, $obj);
    ...
    public function insertProduct($product, $obj)
    {
        $db = JFactory::getDbo();
    ...
    {
        $db = JFactory::getDbo();
        $formats = array('h' => 'hour', 'd' => 'day', 'm' => 'month', 'y' => 'year');
        foreach ($this->store->notifications as $notification) {
            ...
            foreach ($results as $obj) {
                ...
                $this->sendEmail($sender, $notification->subject, $recipients, $notification->body);
                $this->clear();
                $query = $db->getQuery(true)
    ...
    protected function sendEmail($sender, $subject, $recipients, $body)
    {
        ...
        try {
            $mailer->sendMail($sender[0], $sender[1], $recipients, $subject, $body, true, null, null, $files);
        } catch (Exception $e) {}
