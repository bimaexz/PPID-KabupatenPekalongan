...
class fieldsGenerator
{
    ...
    public function getGroupsHTML($fields, $groups, $fields_data, $productData)
    {
        ...
        if ($this->type == 'products' && !isset($object->{'ba-group-subscription-product'})) {
            ...
            $object->{'ba-group-subscription-product'}->fields = array();
        }
        if ($this->type == 'products' && !isset($object->{'ba-group-subscription-renewal'})) {
            ...
            $object->{'ba-group-subscription-renewal'}->fields = array();
        }
    ...
    protected function getSubscriptionProduct($product)
    {
        ...
        $products = isset($subscription->action) ? $subscription->products : array();
        ...
        $html .= '<i class="zmdi zmdi-plus"></i></div>';
        $html .= '<div class="selected-items-wrapper" style="--placeholder-text:\''.JText::_('ADD_NEW_ITEM').'\';">';
        $groups = isset($subscription->action) ? $subscription->groups : array();
    ...
    protected function getSubscriptionRenewal($product)
    {
        ...
        $html .= '<i class="zmdi zmdi-plus"></i></div>';
        $html .= '<div class="selected-items-wrapper" style="--placeholder-text:\''.JText::_('ADD_NEW_ITEM').'\';">';
        $products = isset($subscription->upgrade) ? $subscription->upgrade : array();
        foreach ($products as $id) {
            $page = $this->getSubscriptionPage($id);
            if (!$page) {
                continue;
    ...
    protected function getProductPricingFields($product)
    {
        $keys = array(
            'price' => array(
                ...
                'class' => 'trigger-variations-table one-third-width'
                ),
            'sale_price' => array(
                'type' => 'price',
                'required' => false,
    ...
    {
        $html = '<div class="blog-post-editor-options-group '.$className.'" data-field-key="'.$key;
    ...
    protected function renderUrl($field, $options, $value)
    {
        ...
        if (empty($options->label_type)) {
            $str .= '<div class="ba-url-field-label-wrapper"><label>'.JText::_('LABEL');
            $str .= '</label><input type="text" name="'.$field->id;
            $str .= '" data-name="label" value="'.$data->label.'"></div>';
        }
        $str .= '<div class="link-picker-container"><label>';
        $str .= JText::_('LINK').'</label><input type="text" name="'.$field->id;
        $str .= '"  data-name="link" value="'.$data->link.'">';
    ...
    {
        $str = '<div class="container-icon"><input type="text" class="open-calendar-dialog" name="';
    ...
    protected function renderImage($field, $options, $value)
    {
        ...
        $src = !gridboxHelper::isExternal($data->src) ? JUri::root().$data->src : $data->src;
        $img = !empty($data->src) ? 'background-image:url('.$src.')"' : '';
        $str = '<div class="select-image-field-wrapper"><div class="ba-image-field-label-wrapper"><label>';
        $str .= JText::_('SELECT').'</label><div>';
        ...
        $str .= ' data-value="'.$data->src.'"><i class="zmdi zmdi-camera"></i><div class="image-field-tooltip" style="';
        $str .= $img.'"></div>';
    ...
    protected function renderVideo($field, $options, $value)
    {
        ...
        $str = '<div class="field-sorting-wrapper"><div class="ba-field-video-source-wrapper"><label>';
        $str .= JText::_('VIDEO_SOURCE').'</label>';
