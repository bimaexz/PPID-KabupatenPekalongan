...
abstract class gridboxHelper
{
    ...
    public static function getStoreWishlist($id)
    {
        ...
        foreach ($wishlist->products as $product) {
            ...
            $wishlist->quantity++;
            $product->prices = self::prepareProductPrices($product->product_id, $product->data->price, $product->data->sale_price, $product->variation);
    ...
    public static function getStoreCart($id)
    {
        ...
        foreach ($cart->products as $i => $product) {
            ...
            if ($product->promo) {
                ...
                $value = $cart->promo->discount;
                $unit = $cart->promo->unit;
                $discount = $unit == '%' ? $price * ($value / 100) : $value / $promoProducts;
                $price -= $discount;
        ...
        foreach ($discounts as $sale) {
            if ($cart->total < $sale->cart_discount) {
                continue;
            }
            $discount = $sale->unit == '%' ? ($cart->total - $cart->discount) * ($sale->discount / 100) : $sale->discount;
            $cart->discount += $discount;
            $cart->total -= $discount;
            break;
        }
    ...
    public static function getUserAvatar($email, $key)
    {
        $avatar = '';
        if (!empty($email)) {
            $db = JFactory::getDbo();
    ...
    public static function getCommentsLoginedUserHTML($obj, $type)
    {
        if (empty($obj->avatar) && $type == 'comments-box') {
            $avatar = self::getUserAvatar($obj->email, 'enable_gravatar');
        } else if (empty($obj->avatar) && $type == 'reviews') {
            $avatar = self::getUserAvatar($obj->email, 'reviews_enable_gravatar');
        } else {
            $avatar = $obj->avatar;
        }
    ...
    public static function getComments($id, $parent = 0, $level = 0, $replyName = '')
    {
        ...
        foreach ($comments as $comment) {
            if (empty($comment->avatar)) {
                ...
                $avatar = self::getUserAvatar($comment->email, 'enable_gravatar');
            } else {
    ...
    public static function getReviewById($id, $replyName = '')
    {
        ...
        if (empty($comment->avatar)) {
            ...
            $avatar = self::getUserAvatar($comment->email, 'reviews_enable_gravatar');
        } else {
            $avatar = $comment->avatar;
        }
        $message = str_replace("\n", '<br>', $comment->message);
    ...
    public static function minifyJs($body)
    {
        ...
        pq('head link[type="image/vnd.microsoft.icon"]:first')->after($str);
        $body = $doc->htmlOuter();
    ...
    public static function setBreakpoints()
    {
        ...
        self::$accessibility = JPluginHelper::isEnabled('system', 'accessibility');
    ...
    public static function getFavicon()
    {
        ...
        $favicon = 'components/com_gridbox/assets/images/favicon.ico';
    ...
    public static function loadGridboxLanguage()
    {
        ...
        $result = array('ERROR' => JText::_('ERROR'), 'YEAR' => JText::_('JYEAR'), 'MONTH' => JText::_('JMONTH'));
        if (JFile::exists($path)) {
    ...
    public static function checkPlugin($title)
    {
        $default = ['bagallery' => 1, 'baforms' => 1, 'modules' => 1, 'recent-posts' => 1, 'fields-filter' => 1,
            ...
            'related-posts-slider' => 1, 'recently-viewed-products' => 1, 'currency-switcher' => 1];
    ...
    public static function getSubmissionForm($app_id)
    {
        $html = '';
        if (empty($html)) {
    ...
    public static function checkDOM($html, $obj)
    {
        ...
        foreach (pq('.ba-item') as $value) {
            $id = pq($value)->attr('id');
            if (isset($obj->{$id}) && isset($obj->{$id}->desktop->appearance->effect)) {
        ...
        foreach (pq('.ba-section, .ba-row, .ba-grid-column') as $value) {
            $id = pq($value)->attr('id');
            if (isset($obj->{$id})) {
                if (isset($obj->{$id}->desktop->animation->effect)) {
        ...
        foreach (pq('.ba-item-feature-box') as $value) {
            ...
            foreach ($obj->{$id}->items as $item) {
                if (($view != 'gridbox' && isset($item->unpublish) && $item->unpublish) || empty($item->button->href)) {
                    continue;
                }
                $link = self::prepareGridboxLinks($item->button->href);
                pq($value)->find('.ba-feature-box:nth-child('.$i++.')')->find('.ba-feature-button a')->attr('href', $link);
            }
        }
        ...
        foreach (pq('.ba-item-content-slider') as $value) {
            ...
            foreach ($obj->{$id}->slides as $key => $slide) {
                if (($view != 'gridbox' && isset($slide->unpublish) && $slide->unpublish) || empty($slide->link->href)) {
                    continue;
                }
                ...
                $link = self::prepareGridboxLinks($link);
                pq($value)->find('> .slideshow-wrapper > ul > .slideshow-content > li:nth-child('.$i++.')')
        ...
        foreach (pq('.ba-item-submission-form') as $value) {
            $id = pq($value)->attr('id');
            $str = self::getSubmissionForm($obj->{$id}->app);
            pq($value)->find('.ba-submission-form-wrapper')->html($str);
        }
        ...
        foreach (pq('.ba-item-categories') as $value) {
            ...
            $items = self::getBlogCategories($catApp, 0, $counter, $desktop->view->sub);
        ...
        foreach (pq('.ba-item-search, .ba-item-store-search') as $value) {
            $id = pq($value)->attr('id');
            $url = '';
            $defaultMenuItem = self::getDefaultMenuItem();
            $key = 'query';
            if (isset($obj->{$id}->app) && $obj->{$id}->app != '*'  && $obj->{$id}->app != 'multiple'
                ...
                $url = $system ? 'index.php?option=com_gridbox&view=system&id='.$system->id : '';
            }
            if (!empty($url) && strpos($url, 'Itemid') === false && !empty($defaultMenuItem)) {
                $url .= '&Itemid='.$defaultMenuItem;
            }
            if (!empty($url)) {
                $url .= '&'.$key.'=';
                $url = JRoute::_($url);
        ...
        foreach (pq('.ba-item-add-to-cart') as $value) {
            $itemId = pq($value)->attr('id');
            self::$editItem = $obj->{$itemId};
            $out = self::getAddToCartHTML($view, $page->id, $input);
    ...
    public static function getAddToCartHTML($view, $id, $input)
    {
            ...
            if ($key != 'rating') {
                ...
                foreach ($fields as $field) {
                    if (isset($field->product) && $field->field_type == 'price') {
                        $query = self::getFilterProductsQuery($app, $db);
                        $db->setQuery($query);
                        $list = $db->loadObjectList();
                        foreach ($list as $obj) {
    ...
    public static function getBlogPostIntro()
    {
        ...
        if (!empty($intro_image) || $view == 'gridbox') {
            ...
            $obj->image .= str_replace(' ', '%20', $item->intro_image).');"></div></div>';
        } else {
            $obj->image = '';
        }
    ...
    public static function getBlogCategories($id, $parent = 0, $counter = true, $sub = true)
    {
        $db = JFactory::getDbo();
        $user = JFactory::getUser();
        $groups = $user->getAuthorisedViewLevels();
        ...
        foreach ($items as $item) {
            ...
            if ($sub) {
                $item->childs = self::getBlogCategories($id, $item->id, $counter);
            } else {
                $item->childs = array();
            }
    ...
    public static function getRecentReviews($id, $sorting, $limit, $max, $category = '')
    {
        ...
        foreach ($reviews as $review) {
            ...
            if (empty($review->avatar)) {
                ...
                $avatar = self::getUserAvatar($review->email, 'reviews_enable_gravatar');
            } else {
    ...
    {
        $date = JHtml::date($created, self::$dateFormat, null);
    ...
    public static function getRecentPostsHTML($page, $out, $max)
    {
            ...
            foreach ($fields as $field) {
                ...
                if (empty($field->value)) {
                    ...
                    if (!empty($field->value)) {
                        ...
                        if (is_numeric($field->value) && isset($desktopFiles->{$field->value})) {
                            $desktopFile = $desktopFiles->{$field->value};
                            $src = 'components/com_gridbox/assets/uploads/app-'.$desktopFile->app_id.'/'
        ...
        if (is_object(self::$editItem) &&
            ...
            } else if ($isproduct && empty($productImages) && !empty($page->intro_image)) {
        ...
        } else if (!empty($page->intro_image) && (($desktop && $desktop->view->image) || !$desktop)) {
        ...
        if (($desktop && $desktop->view->date) || !$desktop) {
            $date = '<span>'.self::getPostDate($page->created).'</span>';
    ...
    {
        $str = '';
        $date = '<span>'.self::getPostDate(date('Y-m-d')).'</span>';
        for ($i = 0; $i < 6; $i++) {
    ...
    public static function getItemsFilter($id)
    {
        ...
        foreach ($appFields as $appField) {
            ...
            $str .= '<div class="ba-field-filter-label"><span>'.$label
            ...
            if ($appField->field_type == 'price') {
                ...
                $str .= '<input type="text" class="open-calendar-dialog" readonly data-format="M d, Y" name="'.$label.'" data-value="';
                $str .= (isset($object->{$label}) ? $object->{$label}[0] : '').'" value="'.(isset($object->{$label}) ? $dates[0] : '').'">';
                $str .= '</div>';
                $str .= '<span class="ba-field-filter-date-delimiter">-</span>';
                $str .= '<div><div class="icons-cell"><i class="zmdi zmdi-calendar-alt"></i></div>';
                $str .= '<input type="text" class="open-calendar-dialog" readonly data-format="M d, Y" name="'.$label.'" data-value="';
    ...
    public static function getFieldsGroups($id)
    {
        ...
        $productsGroups = array('ba-group-product-pricing', 'ba-group-product-variations', 'ba-group-related-product',
            'ba-group-digital-product', 'ba-group-subscription-product', 'ba-group-subscription-renewal');
