...
class gridboxViewEditor extends JViewLegacy
{
    ...
    public function display($tpl = null)
    {
        ...
        if ($this->edit_type == 'post-layout') {
            ...
            $this->link = $page ? gridboxHelper::getGridboxPageLinks($page->id, $this->item->type,
                    $id, $page->page_category) : '';
        ...
        } else if ($this->edit_type == 'system' && $this->item->type != 'preloader' && $this->item->type != '404'
            && $this->item->type != 'offline') {
            $this->link = 'index.php?option=com_gridbox&view=system&id='.$id;
        ...
        if (isset ($this->item->app_id) && !empty($this->item->app_id)) {
            $categoryList = $this->getCategoryList($this->item->app_id);
    ...
    protected function getCategoryList($app_id, $id = 0, $level = 0)
    {
        $db = JFactory::getDbo();
        ...
        foreach ($categories as $category) {
            ...
            $data[] = $category;
            $sub = $this->getCategoryList($app_id, $category->id, $level + 1);
