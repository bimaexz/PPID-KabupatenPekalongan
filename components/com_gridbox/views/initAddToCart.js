...
app.addToCart = {
    ...
    showExtraNotice: function(parent){
        parent.find('.ba-add-to-cart-extra-option').each(function(){
            if (this.dataset.required == 1 && (!this.querySelector('.active') && !this.querySelector('.ba-add-to-cart-attachment'))
    ...
    updateExtraPrice: function(obj, extra){
        ...
        extra.closest('.ba-add-to-cart-wrapper').querySelectorAll('.ba-add-to-cart-quantity').forEach((qty) => {
            qty.classList[hasFileQty ? 'add' : 'remove']('file-quantity-enabled');
            if (hasFileQty) {
                qty.querySelector('input').value = fileQty;
            }
        });
    },
