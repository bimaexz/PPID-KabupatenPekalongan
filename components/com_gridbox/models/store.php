...
class gridboxModelStore extends JModelItem
{
    ...
    public function submitRobokassa($id)
    {
        ...
        if (isset($robokassa->params->fiscalization) && $robokassa->params->fiscalization) {
            ...
            $json->payment_method = $robokassa->params->payment_method;
            $json->payment_object = $robokassa->params->payment_object;
            $json->tax = $robokassa->params->tax;
...
<?php
    ...
    public function submitMono($id)
    {
        ...
        $headers = array('X-Token: '.$mono->params->token, 'Content-Type: application/json');
        $curl = curl_init();
        $options = array();
    ...
    public function submitMollie($id)
    {
        ...
        $currency = gridboxHelper::$storeHelper->getDefaultCurrency();
        $array = array(
            "amount" => array("currency" => $currency->code, "value" => $price),
            "description" => $name,
...
<?php
    ...
    public function submitBarion($id)
    {
        ...
        $price = gridboxHelper::preparePrice($order->total, '', '.', $n);
...
<?php
    ...
    public function submitLiqpay($id)
    {
        ...
        $orderId = time();
        $currency = gridboxHelper::$storeHelper->getDefaultCurrency();
        $params = array('action' => 'pay', 'amount' => $order->total, 'currency' => $currency->code,
...
?>
        <form id="payment-form" action="<?php echo $url; ?>" method="post">
            ...
            <input type="hidden" name="x_receipt_link_url" value="<?php echo JUri::root()."index.php?option=com_gridbox&task=store.setOrder&time=".time(); ?>">
...
<?php 
    ...
    public function updateProductQuantity($id, $cart_id, $quantity)
    {
        ...
        $product = $this->getProduct($cart->id, $data->product_id, $product->var, $product->extra, 0, '', 0);
        $product->quantity = 0;
        $this->setProductQuantity($product, $data, $quantity);
        gridboxHelper::updateStoreCart($cart);
    }

    public function addProductToCart($id, $cid, $q, $var = '', $extra = '{}', $renew = 0, $plan = '', $up = 0, $attachments = [])
    {
        ...
        if ($data->stock == '' || $data->stock * 1 >= $min) {
            ...
            $product = $this->getProduct($cart->id, $data->product_id, $var, $extra, $renew, $type, $plan, $up);
    ...
    public function moveProductFromWishlist($id, $product_id, $cart_id)
    {
        ...
        $db = JFactory::getDbo();
        $query = $db->getQuery(true)
            ->select('wp.product_id AS p_id, wp.variation AS var, wp.extra_options AS extra, wp.wishlist_id AS w_id')
            ->from('#__gridbox_store_wishlist_products AS wp')
            ->where('wp.id = '.$product_id);
        $db->setQuery($query);
        $product = $db->loadObject();
        $attachments = $this->getAttachments($product->p_id, 0, $product_id, $product->w_id);
        $this->addProductToCart($product->p_id, $cart_id, $qty, $product->var, $product->extra, 0, '', 0, $attachments);
        $query = $db->getQuery(true)
    ...
    public function addProductToWishlist($id, $w_id, $variation = '', $extra = '{}', $attachments = [])
    {
    ...
    public function getProduct($cid, $pid, $var, $extra = '{}', $renew = 0, $type = '', $plan = '', $up = 0)
    {
        $db = JFactory::getDbo();
