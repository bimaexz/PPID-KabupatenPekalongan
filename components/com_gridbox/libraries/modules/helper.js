...
function showSlideshowDesign(search, select)
{
    ...
    switch (search) {
        ...
        case 'field':
            ...
            if (search == 'field' && (app.edit.type == 'login' || app.edit.type == 'checkout-form')) {
    ...
    if (app.edit.type == 'checkout-form' && search != 'field') {
        parent.find('.slideshow-margin-options').addClass('last-element-child');
    } else if (app.edit.type == 'checkout-form') {
...
app.setLinearInput = (input, text) => {
    let match = String(text).match(/\-{0,1}\d+\.{0,1}\d{0,99}/),
        v = match ? match[0] : 0,
        range = input.closest('.ba-settings-item').find('.ba-range').val(v);
    input.val(text);
    ...
    for (var key in app.editor.breakpoints) {
        if (obj[key] && obj[key].shape) {
            if (obj[key].shape.bottom && obj[key].shape.bottom.effect
                && bottomKeys.indexOf(obj[key].shape.bottom.effect) == -1) {
                bottomKeys.push(obj[key].shape.bottom.effect)
            }
            if (obj[key].shape.top && obj[key].shape.top.effect
                && topKeys.indexOf(obj[key].shape.top.effect) == -1) {
