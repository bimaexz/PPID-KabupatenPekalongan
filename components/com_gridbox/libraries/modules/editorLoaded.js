...
app.shortCutsExecute = function(event){
    ...
    if (flag = event.keyCode == 83 && (event.ctrlKey || event.metaKey) && !event.altKey) {
    ...
    } else if (flag = event.originalEvent && event.originalEvent.code == 'Tab' && event.shiftKey) {
        let btn = $g('.responsive-context-menu span[data-view="'+app.view+'"]').next();
        if (btn.length == 0) {
            btn = $g('.responsive-context-menu span[data-view="desktop"]');
        }
        btn.trigger('mousedown');
    } else if (flag = event.keyCode == 67 && (event.ctrlKey || event.metaKey) && event.shiftKey && app.pageStructure) {
    ...
    } else if (flag = event.keyCode == 86 && (event.ctrlKey || event.metaKey) && app.pageStructure) {
        let li = app.pageStructure.modal.find('.ba-page-structure-list li.active')[0],
            context = app.pageStructure.context.checkMenu(li);
        $g(context).find('[data-action="paste"]').trigger('mousedown');
    } else if (flag = event.keyCode == 46 && !(event.ctrlKey || event.metaKey) && app.pageStructure) {
        let li = app.pageStructure.modal.find('.ba-page-structure-list li.active')[0],
            action = 'delete',
            context = app.pageStructure.context.checkMenu(li);
        if (app.pageStructure.context.items[0].hasClass('ba-grid-column')) {
        ...
        $g(context).find('[data-action="'+action+'"]').trigger('mousedown');
    } else if (flag = event.keyCode == 88 && (event.ctrlKey || event.metaKey) && app.pageStructure) {
        let li = app.pageStructure.modal.find('.ba-page-structure-list li.active')[0],
            action = 'cut',
            context = app.pageStructure.context.checkMenu(li);
        if (app.pageStructure.context.items[0].hasClass('ba-grid-column')) {
...
app.cp = {
    ...
    show: () => {
        let obj = null,
            cp = app.cookie.get('modal-cp-position');
        if (cp) {
            obj = JSON.parse(cp);
            obj.visible = true;
            cp = JSON.stringify(obj);
            app.cookie.set('modal-cp-position', cp);
        }
    ...
    close: (modal) => {
        modal.modal('hide');
        app.cp.set(modal, 'visible', false);
...
app.editorLoaded = function(){
    $g('.select-modal-cp-position').on('customAction', function(){
        ...
        app.cp.set(this.closest('.ba-modal-cp'), 'position', position);
    }).each(function(){
