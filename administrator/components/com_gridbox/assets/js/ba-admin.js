...
var app = {
        ...
        sendBackgroundRequests: function(){
            ...
            if (document.body.dataset.joomlaVersion == 4) {
                fetch('index.php?option=com_gridbox&task=gridbox.fixDB');
            }
        },
        ...
        getProductSortingHTML: function(obj, quantity, symbol, position){
            ...
            str += '</div>';
            if (obj.product_type != 'digital' && obj.product_type != 'subscription') {
...
function prepareCouponApplies(value)
{
    ...
    document.querySelector('.discount-cart-total').style.display = value == 'cart' ? '' : 'none';
}
...
app.fetch = async function(url, data){
...
app.showSystemSettings = function(obj){
    ...
    if (obj.type == '404') {
        modal.find('.page-enable-header').prop('checked', obj.options.enable_header)
            .closest('.error-page-settings').css('display', '');
    } else {
        modal.find('.error-page-settings').hide();
    }
...
app.csv = {
    ...
    importCSV: function(){
        ...
        app.csv.modal.modal('hide');
        makeFetchRequest(app.csv.url+'importCSV', data).then(function(json){
            setTimeout(function(){
                notification.removeClass('notification-in').addClass('animation-out');
                setTimeout(function(){
...
app.exportXML = function(obj){
    ...
    return new Promise(function(resolve, reject) {
        makeFetchRequest('index.php?option=com_gridbox&view=pages&task=pages.exportXML', data).then(function(json){
            let iframe = document.createElement('iframe');
            iframe.style.display = 'none';
            iframe.src = 'index.php?option=com_gridbox&view=pages&task=pages.download';
            document.body.append(iframe);
            resolve();
        });
    });
...
app.associations = {
    ...
    save: function(){
        ...
        this.modal.find('.language-associations-group .association-page').each(function(){
            if (this.dataset.id) {
...
document.addEventListener('DOMContentLoaded', function(){
    ...
    $g('#post-tags-dialog .modal-body').on('click', '.ba-settings-input-type', function(){
    ...
    $g('#create-new-order-dialog').on('show', function(){
        ...
        iframe.src = JUri+'administrator/index.php?option=com_gridbox&view=orders&layout='+this.dataset.layout+'&tmpl=component&id='+this.dataset.id;
        iframe.onload = function(){
            ...
            if (app.cart.order_id) {
                ...
                app.cart.modal.find('.order-shipping-method').each(function(){
                    let select = this.querySelector('select'),
                        ...
                        carrier = app.cart.shipping.carrier != 0 ? $g('.enter-carrier-address').val().trim() : '';
    ...
    $g('#store-email-options .sorting-container').on('click', '.notification-sorting-item', function(event){
        ...
        for (let ind in obj) {
            if (ind == 'body') {
                ...
                if (obj.status == 'stock') {
                    status = app._('OUT_OF_STOCK');
                } else if (obj.status == 'reminder') {
                    status = app._('EXPIRATION_RENEWAL_REMINDER');
                }
        ...
        if (obj.status == 'stock' || obj.status == 'reminder') {
            modal.find('.notification-status-option, .notification-recipient-option').addClass('disabled');
        } else {
            modal.find('.notification-status-option, .notification-recipient-option').removeClass('disabled');
        }
    ...
    $g('.apply-email-notification').on('click', function(event){
        ...
        modal.find('.ba-options-group-element:not(.ba-hide-element) [data-key]').each(function(){
            ...
            } else if (this.dataset.group == 'delay') {
                obj.delay[key] =  this.type == 'checkbox' ? this.checked : this.value;
            } else {
    ...
    $g('.apply-system-settings').on('click', function(event){
        ...
        app.fetch('index.php?option=com_gridbox&task=system.applySettings', {
            ...
            language: modal.find('.page-language').val(),
    ...
    function showPageSettings(obj, tr)
    {
        ...
        let image = !app.isExternal(obj.intro_image) ? JUri+obj.intro_image : obj.intro_image;
        $g('#settings-dialog .intro-image').val(obj.intro_image).parent().find('.image-field-tooltip').css({
            'background-image': obj.intro_image ? 'url('+image.replace(/\s/g, '%20')+')' : ''
        });
    ...
    function createCalendar($this)
    {
        if ($this.dataset.type == 'range-dates') {
            ...
            $this.disableFunc = function(date){
        ...
        $g(btn).on('click', function(){
            gridboxCalendar.show($this);
        });
        $g($this).on('update', function(){
            if (this.dataset.type == 'range-dates' && this.dataset.key == 'to' && this.range.value == '') {
                this.range.value = this.value;
                this.value = '';
            }
            if (this.dataset.type == 'range-dates' && this.range.value && this.value && this.dataset.action == 'filter') {
                createAjax();
    ...
    function loadPage(firstLoading)
    {
        ...
        $g('.ba-create-tags').on('mousedown', function(event){
            event.preventDefault();
            var modal = $g('#create-new-tag-modal');
            modal.find('.ba-btn-primary').removeClass('active-button');
        ...
        $g('.set-featured-post').on('click', function(){
            var id = $g(this).closest('tr').find('td.select-td input[type="checkbox"]').val();
            $g.ajax({
                type : "POST",
                dataType : 'text',
    ...
    $g('.reset-share-image').on('click', function(){
        $g(this).parent().find('input[type="text"]').val('');
