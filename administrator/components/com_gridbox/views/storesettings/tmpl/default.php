...
$obj = new stdClass();
$obj->title = JText::_('OUT_OF_STOCK');
$obj->color = '#ff4f49';
$obj->key = 'stock';
$statuses['stock'] = $obj;
$obj = new stdClass();
$obj->title = JText::_('EXPIRATION_RENEWAL_REMINDER');
$obj->color = '#ff4f49';
$obj->key = 'reminder';
$statuses['reminder'] = $obj;
...
<?php
                                            foreach ($notifications as $key => $notification) {
                                                if (isset($statuses[$notification->status])) {
                                                    $status = $statuses[$notification->status];
                                                } else {
                                                    $status = null;
                                                }
                                                $canDelete = !(in_array($notification->status, $system));
?>
...
<script>
    ...
    app.statuses = <?php echo json_encode($statuses); ?>;
