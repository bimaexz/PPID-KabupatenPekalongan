...
class JmgQuestionnaireViewQuestion extends JViewLegacy
{
	...
	protected function addToolbar()
	{
		...
		JLoader::register('JmgQuestionnaireHelper', JPATH_ADMINISTRATOR . '/components/com_jmgquestionnaire/helpers/jmgquestionnaire.php');
		JToolbarHelper::title($isNew ? JText::_('COM_JMGQUESTIONNAIRE_QUESTION_NEW_TITLE') : JText::_('COM_JMGQUESTIONNAIRE_QUESTION_EDIT_TITLE'));
		...
		else
		{
			...
			{
				JToolbarHelper::versions('com_jmgquestionnaire.question', $this->item->id);
			}
			JToolbarHelper::cancel('question.cancel', 'JTOOLBAR_CLOSE');
			$layout = new JLayoutFile('toolbar.toquestionnaire');
			$bar  = JToolbar::getInstance('toolbar');
			$bar->appendButton('Custom', $layout->render(array()), 'toquestionnaire');
		}
