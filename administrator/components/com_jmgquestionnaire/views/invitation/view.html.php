...
/**
 * View to edit a invitation.
 *
 * @since  1.5
 */
class JmgQuestionnaireViewInvitation extends JViewLegacy
{
	protected $form;
	protected $item;
	protected $state;
	...
	protected function addToolbar()
	{
		...
		//JToolbarHelper::apply('invitation.apply');
		//JToolbarHelper::save('invitation.save');
		//JToolbarHelper::save2new('invitation.save2new');
		
		if (empty($this->item->id))
		{
			JToolbarHelper::cancel('invitation.cancel');
		}
		else
		{
			if (JComponentHelper::isEnabled('com_contenthistory') && $this->state->params->get('save_history', 0) && $canDo->get('core.edit'))
			{
				JToolbarHelper::versions('com_jmgquestionnaire.invitation', $this->item->id);
			}
			JToolbarHelper::cancel('invitation.cancel', 'JTOOLBAR_CLOSE');
		}
