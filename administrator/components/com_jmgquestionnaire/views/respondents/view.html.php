...
/**
 * View class for a list of respondents.
 *
 * @since  1.6
 */
class JmgQuestionnaireViewRespondents extends JViewLegacy
{
	...
	public function display($tpl = null)
	{
		...
		JmgQuestionnaireHelper::addSubmenu('respondents');
	...
	protected function addToolbar()
	{
		...
		JToolBarHelper::title(JText::_('COM_JMGQUESTIONNAIRE_RESPONDENTS_TITLE'));
		
		if (count($user->getAuthorisedCategories('com_jmgquestionnaire', 'core.create')) > 0)
		{
			//JToolbarHelper::addNew('respondent.add');
		}
		
		if ($canDo->get('core.edit.state'))
		{
			//JToolbarHelper::publish('respondents.publish', 'JTOOLBAR_PUBLISH', true);
			//JToolbarHelper::unpublish('respondents.unpublish', 'JTOOLBAR_UNPUBLISH', true);
		}
		
		if ($this->state->get('filter.published') == -2 && $canDo->get('core.delete'))
		{
			JToolbarHelper::deleteList('JGLOBAL_CONFIRM_DELETE', 'respondents.delete', 'JTOOLBAR_EMPTY_TRASH');
		}
		elseif ($canDo->get('core.edit.state'))
		{
			JToolbarHelper::trash('respondents.trash');
		}
